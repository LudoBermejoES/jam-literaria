/**
 * Módulo para la lógica de selección de ideas y gestión de votaciones
 * Implementa el algoritmo completo para seleccionar las 3 ideas ganadoras,
 * manejar desempates y gestionar las rondas de votación.
 */

/**
 * Representa una idea con su información y conteo de votos
 */
...
/**
 * Representa el estado de una sesión
 */
export interface Session {
  ...
  votosNuevaRonda?: Record<string, string[]>;
  ideasFinales?: Idea[];
  fechaFinalizacion?: Date;
}

/**
 * Resultado de la determinación de acción siguiente
 */
 ...
 * @param {Array} ideas - Array de objetos con formato {id, content, votos, authorId}
 ...
 */
export function determinarAccionSiguiente(ideas: Idea[]): ResultadoAccion {
  ...
  if (gruposOrdenados.length > 0 && gruposOrdenados[0].ideas.length === 3) {
    return { 
      accion: 'FINALIZAR', 
  ...
  if (gruposOrdenados.length > 0 && gruposOrdenados[0].ideas.length === 2 && gruposOrdenados.length > 1) {
  ...
  if (gruposOrdenados.length > 0 && gruposOrdenados[0].ideas.length === 1) {
    const ideaConMasVotos = gruposOrdenados[0].ideas[0];
    
    // Subcaso 3.1: En segunda posición hay exactamente 2 ideas empatadas
    if (gruposOrdenados.length > 1 && gruposOrdenados[1].ideas.length === 2) {
      return {
        accion: 'FINALIZAR',
    ...
    if (gruposOrdenados.length > 1 && gruposOrdenados[1].ideas.length === 1) {
      const ideaSegundoLugar = gruposOrdenados[1].ideas[0];
      
      // Si en tercera posición hay exactamente 1 idea, tenemos las 3 elegidas
      if (gruposOrdenados.length > 2 && gruposOrdenados[2].ideas.length === 1) {
        return {
          accion: 'FINALIZAR',
 ...
 */
export function finalizarSeleccion(
  ideasElegidas: Idea[],
  session: Session
): Session {
  return {
